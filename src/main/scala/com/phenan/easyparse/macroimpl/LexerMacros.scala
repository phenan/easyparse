...
class LexerMacros (val c: whitebox.Context) extends MacroCommons {
  ...
  private def buildLexer (info: LexerInfo): Tree = {
    val lexers = info.lexerPatterns.map(pattern => pattern.lexer)
    ...
    val patterns = info.lexerPatterns.map(pattern => pattern.pattern)
